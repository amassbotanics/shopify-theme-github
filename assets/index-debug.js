...
var mapSettings = require("./map-settings.js");
...
var initMap = function initMap() {
  ...
  $("body").on("click", "[data-location-id]", function (e) {
    e.preventDefault();
    ...
    map.markers.forEach(function (marker) {
      if (marker.locationId == locationId) {
        ...
        var scroll = new _smoothScroll["default"]();
        scroll.animateScroll($("#top")[0]);
      }
    });
  });

  var mapBoundsChanged = function mapBoundsChanged() {
    markersInView = [];
      ...
      for (var i = 0; i < map.markers.length; i++) {
        if (map.getBounds().contains(map.markers[i].getPosition())) {
          if (typeof locations[i] !== "undefined") {
            markersInView.push(locations[i]);
          }
        }
      }

      if (typeof map.markerClusterer !== "undefined" && map.markerClusterer !== null) {
        map.markerClusterer.repaint();
      }

      updateTextResults(markersInView);
    }
  };

  var updateTextResults = function updateTextResults(markersInView) {
    $(".location-text-results").html("");
    var ctaClass = "dark";

    if ($("body").hasClass("storytellers-site")) {
      ctaClass = "white";
    }

    $(markersInView).each(function (idx, location) {
      var locationMarkup = "\n        <div class=\"col-10 col-lg-3 link-block\" data-category=\"".concat(location.category, "\">\n        <div class=\"text wysiwyg-text h-100 d-flex flex-column\">\n          <p class=\"type--b-em\"><a href=\"#\" data-location-id=\"").concat(location.id, "\">").concat(location.name, "</a></p>\n          <p class=\"wysiwyg-content\">\n            ").concat(location.address, "<br>\n            ").concat(location.phone, "\n          </p>\n          <p class=\"cta mt-auto\">\n            <a href=\"").concat(location.url, "\" target=\"_blank\" class=\"cta caret-link type--cta ").concat(ctaClass, "\">Visit Website<i class=\"caret\"></i></a>\n          </p>\n        </div>\n      </div>\n      ");
      $(".location-text-results").append(locationMarkup);
    });
  };

  var updateMapResults = function updateMapResults() {
    var selectedType = $('input[name="location_type"]:checked').val();

    if ($(".location-text-results[data-show]").length) {
      $(".location-text-results[data-show]").attr("data-show", selectedType);
    }

    map.markerClusterer.setIgnoreHidden(true);
    ...
    for (var i = 0; i < map.markers.length; i++) {
      if (map.markers[i].locationType == selectedType) {
  ...
  function new_map($el) {
    var mapStyles = mapSettings.styles,
        clusterImage = ""; // vars

    var args = {
      zoom: defaultZoom,
      center: new google.maps.LatLng(34.09523, -118.3727),
    ...
    var storeUrl = "https://storemapper-herokuapp-com.global.ssl.fastly.net/api/users/5340/stores.js";
    $.getJSON(storeUrl, function (json) {
      $(json.stores).each(function (idx, store) {
        locations.push(store);
        add_marker(store, map);
      });
      var clusterStyles = [{
        url: $(".map").attr("data-cluster-icon"),
        textColor: "white",
        textSize: 16,
        height: 40,
        width: 40,
      ...
      if (map.markers.length > 1) {
        map.markerClusterer = new _markerclustererplus["default"](map, map.markers, {
  ...
  function add_marker(location, map) {
    var markerColor = "#000000"; // var

    var latlng = new google.maps.LatLng(location.latitude, location.longitude); // create marker
    ...
    var marker = new google.maps.Marker({
      ...
      optimized: true,
      icon: {
        url: $(".map").attr("data-marker-icon"),
        anchor: new google.maps.Point(0, 16.5),
    ...
    var infoWindowContent = "\n    <h3>".concat(location.name, "</h3>\n    <p>").concat(location.address, "</p>\n    <p>").concat(location.category, "</p>\n    <p><a href=\"").concat(location.url, "\" target=\"_blank\">").concat(location.url, "</a></p>\n    ");
  ...
  function center_map(map) {
    // vars
    bounds = new google.maps.LatLngBounds(); // loop through all markers and create bounds
  ...
  if ($(".locator-map-container").length) {
    if ($(window).width() < 992) {
      defaultZoom = 14;
    }

    map = new_map($(".locator-map-container .map"));
  }
};
...
var updateNav = function updateNav() {
  ...
  if (scrollDirection == "up") {
    $("body").removeClass("scroll-direction-down").addClass("scroll-direction-up");
  } else {
    if (scrollPercent > 0) {
      $("body").removeClass("scroll-direction-up").addClass("scroll-direction-down");
    }
  }
...
var setupGlobalScrollEvents = function setupGlobalScrollEvents() {
...
var initNav = function initNav() {
  $(".bars").on("touchstart click", function (e) {
    e.preventDefault();

    if ($("body").hasClass("site-header--open")) {
      // unlock scrolling
      $("body").removeClass("site-header--open");
      bodyScrollLock.clearAllBodyScrollLocks();
    } else {
      // lock scrolling
      $("body").addClass("site-header--open");
      disableBodyScroll($(".mobile-nav--links")[0]);
    }
  });
  $(".hover-images").each(function (idx, elem) {
    $(elem).find(".hover-image:first").addClass("active");
  });
  $(".hover-image").each(function (idx, elem) {
    var href = $(this).attr("data-link-url");
    $('.primary-nav--links a[href="' + href + '"]').on("mouseover", function (e) {
      $(this).parents(".child-menu").find(".hover-image.active").removeClass("active");
      $(elem).addClass("active");
    });
  });
  $(".section--post-listing .load-more, .section--links .load-more").on("click", function (e) {
    e.preventDefault();
    $(this).parents(".load-more-container").find(".will-reveal").removeClass("d-none");
    $(this).parents(".row").remove();
  });
};

var initHero = function initHero() {
  $(".hero .slides").each(function (idx, elem) {
    if ($(elem).find(".hero-slide").length > 1) {
      $(elem).flickity({
        pageDots: false,
  ...
  $(".block-carousel").each(function (idx, elem) {
    if ($(elem).find(".slide").length > 3) {
      $(elem).flickity({
        wrapAround: true,
        pageDots: false,
...
var initProductDetails = function initProductDetails() {
  $(".product-details-carousel-images").each(function (idx, elem) {
    if ($(elem).find(".slide").length > 1) {
      var $carousel = $(elem).flickity({
        pageDots: false,
        prevNextButtons: false,
        wrapAround: true,
      ...
      $(".product-details-carousel-nav .thumb").on("click", function (e) {
        e.preventDefault();
        var slideTarget = $(this).attr("data-slide-target");
        $carousel.flickity("select", slideTarget);
      });
    }
  });
};

var initGrids = function initGrids() {// $(".grid").each(function(idx, elem) {
...
var initPostDetails = function initPostDetails() {
  $(".post-body img").each(function (idx, elem) {
    $(elem).wrap('<div class="image"></div>');

    if ($(elem).attr("alt").length) {
      var caption = $(elem).attr("alt");
      $(elem).parents(".image").addClass("with-caption").append('<div class="caption">' + caption + "</div>");
    }
  });
  $(".post-body .image").unwrap("p");
};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  for (var idx = 0; idx < anchors.length; idx += 1) {
    ...
    if (anchors[idx].classList.contains("no-fade") == false) {
...
$(document).ready(function () {
  ...
  var scroll = new _smoothScroll["default"]("a[data-scroll]");
  $(document).on("lazyloaded", function (e) {
    $(window).trigger("resize");
  });
});

},{"./map-settings.js":2,"@google/markerclustererplus":3,"body-scroll-lock":4,"colcade":5,"flickity":15,"flickity-fade":9,"jquery":23,"jquery-bridget":22,"lazysizes":24,"smooth-scroll":25}],2:[function(require,module,exports){
"use strict";
...
},{"fizzy-ui-utils":8,"flickity":15}],10:[function(require,module,exports){
// add, remove cell
( function( window, factory ) {
  // universal module definition
  /* jshint strict: false */
...
},{"./flickity":14,"fizzy-ui-utils":8,"unidragger":26}],14:[function(require,module,exports){
// Flickity main
( function( window, factory ) {
  // universal module definition
  /* jshint strict: false */
...
},{"./flickity":14,"fizzy-ui-utils":8,"unipointer":27}],18:[function(require,module,exports){
// player & autoPlay
( function( window, factory ) {
  // universal module definition
  /* jshint strict: false */
...
},{"./flickity":14,"fizzy-ui-utils":8,"unipointer":27}],20:[function(require,module,exports){
// slide
( function( window, factory ) {
  // universal module definition
  /* jshint strict: false */
